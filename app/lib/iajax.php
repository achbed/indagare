...
global $debug_mail, $debug_mail_prefix, $admin_mail, $admin_mail_prefix;
$debug_mail = '';
$debug_mail_prefix = 'DEBUG: ';
$admin_mail = 'admin@indagare.com';
$admin_mail_prefix = 'NOTICE: ';
...
/*
if ( class_exists( '\indagare\iajax\Handler' ) ) {
    if ( ! empty( $_GET["task"] ) ) {
        $task = htmlspecialchars( $_GET["task"], ENT_QUOTES, 'UTF-8' );
        if ( method_exists( '\indagare\iajax\Handler', $task ) ) {
            return \indagare\iajax\Handler::{$task}();
        }
    }
    die( "no task identified" );
}
...
if ( $task == "signup1" ) {
	...
	/*         $q1 = $acc->user->question_1;
	 ...
	 $m->send('A new application for Indagare', $message, "Indagar <info@indagare.com>");
